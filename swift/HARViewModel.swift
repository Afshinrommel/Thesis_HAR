...
class HARViewModel: ObservableObject {
    ...
    func getFinalCSVPath() -> URL? {
        // ۱. هدر ۱۶ ستونه دقیق
        let header = "timestamp,activity,confidence,roll,pitch,yaw,userAccX,userAccY,userAccZ,gyroX,gyroY,gyroZ,totalAccX,totalAccY,totalAccZ,ground_truth\n"
        ...
        if !finalContent.hasPrefix("timestamp") {
    ...
    private func startNewRecording() {
            ...
            if currentModel == .randomForest {
                csvData = "Timestamp,Label,Confidence,UserAccX,UserAccY,UserAccZ,GravX,GravY,GravZ,Roll,Pitch,Yaw,GyroX,GyroY,GyroZ,Ground_Truth\n"
            }
            else if currentModel == .uciRandomForest || currentModel == .uciRandomForest75 {
                csvData = "Timestamp,Label,Confidence,UserAccX,UserAccY,UserAccZ,TotalAccX,TotalAccY,TotalAccZ,GyroX,GyroY,GyroZ,Ground_Truth\n"
            }
            else {
                csvData = "Timestamp,Label,Confidence,RawAccX,RawAccY,RawAccZ,GyroX,GyroY,GyroZ,Ground_Truth\n"
            }
        }
...
extension HARViewModel: SensorProviderDelegate {
    ...
    func didCollectFullWindow(
            ...
            if self.isRecording {
                        ...
                        // ایجاد یک بلاک متنی برای تمام ۱۲۸ نمونه
                        var windowBlock = ""
                        
                        for i in 0..<userAccX.count {
                            // محاسبه زمان برای هر نمونه (هر نمونه ۰.۰۲ ثانیه فاصله دارد)
                            let sampleTime = baseTime + (Double(i) * 0.02)
                            
                            // ساخت خط ۱۶ ستونه دقیق مطابق با هدر getFinalCSVPath:
                            // timestamp(0), activity(1), confidence(2), roll(3), pitch(4), yaw(5),
                            // userAccX(6), userAccY(7), userAccZ(8), gyroX(9), gyroY(10), gyroZ(11),
                            // totalAccX(12), totalAccY(13), totalAccZ(14), ground_truth(15)
                            let line = String(format: "%.3f,%@,%.1f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%@\n",
                                              sampleTime,
                                              labelToWrite,
                                              self.confidence,
                                              rolls[i], pitches[i], yaws[i],           // 3, 4, 5
                                              userAccX[i], userAccY[i], userAccZ[i],   // 6, 7, 8
                                              gyroX[i], gyroY[i], gyroZ[i],           // 9, 10, 11
                                              rawAccX[i], rawAccY[i], rawAccZ[i],     // 12, 13, 14 (TotalAcc)
                                              groundTruth                              // 15
                            )
                            windowBlock.append(line)
                        }
                        
                        // اضافه کردن کل ۱۲۸ خط به دیتای اصلی
